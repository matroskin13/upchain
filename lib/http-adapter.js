...
class HttpAdapterServer {
    constructor(options, listener, pluginListener) {
    ...
    mapping() {
        ...
        router.get('/__plugin/:plugin', this.usePlugin.bind(this));

        return router;
    }

    usePlugin(req, res) {
        let result = this.pluginListener(req.params.plugin, req.query);

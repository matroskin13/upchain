...
function useEvent(callback, property, input, payload) {
    let result = callback(property, input, payload);

    if (result && 'then' in result) {
        return callback(property, input, payload).catch(e => {
            if (!e.level) {
...
class Service {
    ...
    listener(input, payload) {
        ...
        this.events.forEach(event => {
            ...
            let isAllEvent = event.type === ALL_EVENT;
            let isInputEvent = event.type === INPUT_EVENT;
            let isPayloadEvent = event.type === PAYLOAD_EVENT;

            if ((isInputEvent || isAllEvent) && inputProperty) {
                promises.push(useEvent(event.callback, inputProperty, input, payload));
            }

            if ((isPayloadEvent || isAllEvent) && payloadProperty) {
                promises.push(useEvent(event.callback, payloadProperty, input, payload));
            }
        });
    ...
    all(property, callback, options) {
        this.addEvent(ALL_EVENT, property, callback, options);
    }
